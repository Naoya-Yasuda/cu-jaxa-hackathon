...
JAXA衛星データから得られる植生状態・気象条件・地形情報と、過去のクマ目撃データを機械学習モデルで学習し、**東北3県（秋田・岩手・福島）内のクマ出没リスクをメッシュ単位で予測**するWebアプリケーションを構築する。

### 1.2 背景

- 東北3県ではツキノワグマとニホンヒグマの出没が深刻化
- 2024年度の人身被害統計:
  - 秋田県: 56名（全国最多）
  - 岩手県: 34名
  - 福島県: 20名
...
- 秋田県・岩手県・福島県内の個体群
- 生息地: 秋田駒ヶ岳、白神山地、早池峰山、奥秩父等の山地域
- 保全状況: 東北地方の重要な野生生物保護対象
...
### 2.1 ハイブリッド方式（ベースリスク のみ - MVP）

**MVP段階では月次ベースリスクのみを実装。日次補正は将来の拡張機能とする。**

| レイヤー | 更新頻度 | データソース | 説明 |
|---|---|---|---|
| **ベースリスク** | 月1回 | NDVI変化量、森林マップ、地形 | 植生の季節変化に基づく中長期リスク |
| **日次補正係数** | 将来対応 | 気象データ（気温・降水）、直近の目撃情報 | 天候や直近の活動に基づく短期補正（フェーズ2以降） |
| **最終リスクスコア** | 月1回 | ベースリスク（MVP） | ユーザーに表示するスコア（0〜100） |

### 2.2 設計根拠

- NDVI（植生指数）は日単位では大きく変動しないため、月次更新が合理的
- MVP では衛星データの季節変化によるベースリスク推定に注力
- 気温・降水量による日次補正や直近目撃情報の反映は、データパイプラインの安定化後に段階的に追加
...
| # | データ | ソース | 空間分解能 | 時間分解能 | 取得方法 | 用途 |
|---|---|---|---|---|---|---|
| S1 | **NDVI（植生指数）** | GCOM-C（しきさい）SGLI | ~5km | 日次/月次 | JAXA Earth API（登録不要） | 堅果類の豊凶推定、植生活性度 |
| S2 | **森林・非森林マップ** | ALOS-2 PALSAR-2 FNF | ~1km | 年1回 | JAXA Earth API / G-Portal | 森林域の特定、生息適地判定 |
| S3 | **地表面温度（LST）** | GCOM-C SGLI | ~5km | 日次 | JAXA Earth API | 気温補正、冬眠時期推定（将来用） |

#### 衛星データの空間弁別と時間弁別

**空間弁別（どこが危ないか）**:
- DEM標高・傾斜角（~1km解像度）：地形リスク判定
- FNF森林マップ（~1km解像度）：生息適地判定
- 市街地・河川までの距離（ベクターデータ）：出没適地判定

**時間弁別（いつ危ないか）**:
- NDVI（~5km解像度）：月次の植生活性度トレンド
- LST地表面温度（~5km解像度）：気象相関指標（将来用）
- GSMaP降水量（~10km解像度）：気象相関指標（将来用）
...
| # | データソース | データ形式 | 取得方法 | URL | 備考 |
|---|---|---|---|---|---|
| B1 | **秋田県クマダス** | CSV | オープンデータダウンロード | https://ckan.pref.akita.lg.jp/dataset/050008_shizenhogoka_003 | 秋田県の公式目撃・被害データベース |
| B2 | **福島県ツキノワグマ目撃情報** | Excel | 自治体提供ページからダウンロード | https://www.pref.fukushima.lg.jp/sec/16035b/tukinowaguma-mokugeki.html | 福島県の目撃情報統計 |
| B3 | **岩手県オープンデータ** | CSV/JSON | Webサービス / 手動取得 | https://www.pref.iwate.jp/opendata/1000081/index.html | 岩手県の自然保護課データ |
| B4 | **くまップ（全国）** | JSON | REST API | https://kumapp.net | 参考データ・学習セット拡張用 |
...
source:         TEXT     -- データソース: akita / fukushima / iwate / kumapp
head_count:     INTEGER  -- 頭数（不明の場合NULL）
description:    TEXT     -- 備考
prefecture:     TEXT     -- 都道府県: 秋田 / 岩手 / 福島
```

#### ハッカソン向けデータ取得戦略

```
優先度1: 秋田県クマダスからCSV取得（オープンデータ）
優先度2: 福島県・岩手県の公開ページからExcel取得
優先度3: くまップAPIから全国データを参考取得
優先度4: 不足分はテストデータ生成スクリプトで補完
```
...
|---|---|---|---|---|---|
| W1 | **気温・降水量** | 気象庁アメダス | 10分毎 | JSON API | 日次補正の入力値（将来用） |
| W2 | **積雪深** | 気象庁アメダス | 日次 | JSON API | 冬眠時期の推定（将来用） |
...
| パラメータ | 値 | 根拠 |
|---|---|---|
| 配置範囲 | 秋田・岩手・福島の森林域（標高300〜1500m） | ツキノワグマの主な生息標高帯 |
| 季節パターン | 4〜6月（春）・9〜11月（秋）にピーク | 冬眠明け＆冬眠前の餌探し行動 |
| 地形バイアス | 河川沿い・森林縁部で出現確率UP | 既知のクマ行動パターン |
| 年間件数 | 100〜200件 | 東北3県の近年の実績ベース（2024年度110件人身被害） |
| 空間分布 | 秋田県の山間部・奥羽山脈沿いに集中 | 実際の出没多発地域 |
...
| 項目 | 仕様 |
|---|---|
| 空間単位 | 1km × 1km メッシュ（東北3県 ≈ 約50,000メッシュ） |
| 時間単位 | 月次（MVP：ベースリスク月次更新） |
...
| F4 | **時間** | 月、曜日、祝日フラグ | カレンダー |
| F5 | **履歴** | 同メッシュ過去出没回数、近傍メッシュの直近30日出没数 | 目撃データ (B1-B4) |

### 4.4 ターゲット変数と不均衡対処

- 各メッシュ・各月の **出没有無（0 / 1）** → 二値分類問題
- **ネガティブサンプリング**: 正例1：負例10-20の比率でクラス不均衡対処
...
│   ├── fetch_sightings.py                 # 東北3県データ・くまップAPI取得
...
| 2 | 秋田県クマダス・福島県・岩手県データ取得 | `scripts/fetch_sightings.py` | `data/raw/sightings/{akita,fukushima,iwate}.*` |
...
| 5 | 気象データ取得（将来用） | `scripts/fetch_weather.py` | `data/raw/weather/` |
...
| V5 | 空間妥当性 | 予測マップと実際の出没データを比較 | 秋田県山間部・奥羽山脈沿いが高リスク |
...
| **NDVI時系列ビジュアル** | 東北3県のNDVI季節変化を画像で見せる | 視覚的に衛星データの意味を伝える |
...
| **秋田県クマダス** | https://ckan.pref.akita.lg.jp/dataset/050008_shizenhogoka_003 |
| **福島県ツキノワグマ目撃情報** | https://www.pref.fukushima.lg.jp/sec/16035b/tukinowaguma-mokugeki.html |
| **岩手県オープンデータ** | https://www.pref.iwate.jp/opendata/1000081/index.html |
| くまップ | https://kumapp.net |
...
## 付録B: config.py 設定値リファレンス

```python
# 対象地域の緯度経度範囲 [minlon, minlat, maxlon, maxlat]
BBOX = [139.2, 36.8, 142.1, 40.5]  # 東北3県を包含
BBOX_AKITA = [139.7, 39.0, 140.6, 40.5]
BBOX_FUKUSHIMA = [139.8, 36.8, 141.0, 37.8]
BBOX_IWATE = [139.5, 38.8, 141.2, 40.3]
...
# クラス不均衡対処用ネガティブサンプリング比率
NEG_SAMPLE_RATIO = 15  # 正例1：負例15
```
